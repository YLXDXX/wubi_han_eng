require('frost.cold_word_drop.string')
require("frost.cold_word_drop.metatable")
-- local puts = require("tools/debugtool")
local drop_list = require("frost.cold_word_drop.drop_words")
local hide_list = require("frost.cold_word_drop.hide_words")
local turndown_freq_list = require("frost.cold_word_drop.turndown_freq_words")
local tbls = {
    ['drop_list'] = drop_list,
    ['hide_list'] = hide_list,
...
local function get_record_filername(record_type)
    ...
    if user_distribute_name == '小狼毫' then
        return string.format("%s\\Rime\\lua\\frost\\cold_word_drop\\%s_words.lua", os.getenv("APPDATA"), record_type)
    end
    ...
    if system == "Darwin" then
        filename = string.format("%s/Library/Rime/lua/frost/cold_word_drop/%s_words.lua", os.getenv('HOME'), record_type)
    elseif system == "Linux" then
        filename = string.format("%s/%s/rime/lua/frost/cold_word_drop/%s_words.lua",
            os.getenv('HOME'),
...
local function write_word_to_file(record_type)
    -- local filename = string.format("%s/Library/Rime/lua/frost/cold_word_drop/%s_words.lua", os.getenv('HOME'), record_type)
