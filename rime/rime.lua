...
function single_char(input, env)
	...
	for cand in input:iter() do
		if (not b or utf8.len(cand.text) == 1 or cand.text:find("^%a") or table.vIn(rv_var, input_text) or input_text:find("^z") or input_text:find("^[%u%p]")) then
			yield(cand)
		end
	end
end
...
function number_translator(input, seg)
	...
	if string.match(input,"^(ZZ+%d+)(%.?)(%d*)$")~=nil then
